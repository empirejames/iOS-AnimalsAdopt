...
@interface PartnerDetailViewController (){
    ...
    NSString *title,*i_url,*content,*pic;
...
- (IBAction)btn_use:(id)sender
{
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:i_url]];
}
...
-(void)connectHttp{
    ...
    [receivedData setLength:0];
     ...
     NSString *img_url =[NSString stringWithFormat:@"%@/Query/AppPartnerItem.ashx?id=%@",NSLocalizedString(@"api_ip", @""),indexId];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)_connection {
    ...
    [detail_scrollView setContentSize:CGSizeMake(screenWidth , size.height+ 100)];
...
-(void)showAdvs{
    ...
    for(int i = 0; i < [advsList count]; i ++){
       // UIButton *thumbView = [[UIButton alloc] init];
        ...
        //thumbView.frame = CGRectMake(self.view.frame.size.width * i, 0, self.view.frame.size.width-100, img_scrollView.frame.size.height);
        ...
        //UIImage *urlimage = [UIImage imageWithData:imageData scale:[[UIScreen mainScreen] scale]];
