...
- (void)initialize{
    ...
    if ([[UIDevice currentDevice]userInterfaceIdiom]==UIUserInterfaceIdiomPhone) {
         _pageSize = CGSizeMake(120, 160);
    }
    else if ([[UIDevice currentDevice]userInterfaceIdiom]==UIUserInterfaceIdiomPad)
    {
       _pageSize = CGSizeMake(220, 280);
    }
    
   // _pageSize = self.mSize.size;
    _pageCount = 0;
    _currentPageIndex = 2;
            ...
            for (int i = _visibleRange.location; i < _visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.width) {
                    ...
                    CGFloat pageScale = 1 - (delta / _pageSize.width) * (1 - _minimumPageScale);
                      cell.layer.transform = CATransform3DMakeScale(pageScale+0.3, pageScale+0.3, 1);
                } else {
                    cell.alpha = _minimumPageAlpha;
                    cell.layer.transform = CATransform3DMakeScale(_minimumPageScale+0.3, _minimumPageScale+0.3, 1);
                }
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        switch (orientation) {
            case PagedFlowViewOrientationHorizontal:
                cell.frame = CGRectMake(_pageSize.width * pageIndex+5, 10, _pageSize.width, _pageSize.height);
                break;
            case PagedFlowViewOrientationVertical:
...
- (void)setPagesAtContentOffset:(CGPoint)offset{
    ...
    switch (orientation) {
        ...
        case PagedFlowViewOrientationVertical:{
            ...
            _visibleRange.location = startIndex;
            _visibleRange.length = endIndex - startIndex ;
